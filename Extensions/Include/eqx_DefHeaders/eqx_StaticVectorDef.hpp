...
namespace eqx
{
	...
	{
		runtimeAssert(index >= 0 && index < m_CurSize, "Index Out Of Bounds");
	...
	template <typename t_Held, std::uint8_t Capacity>
		requires std::is_default_constructible_v<t_Held>
	constexpr StaticVector<t_Held, Capacity>::t_CRef
		StaticVector<t_Held, Capacity>::operator[] (t_Size index) const 
		noexcept
	{
		return at(index);
	}
	...
	constexpr void StaticVector<t_Held, Capacity>::resize(t_Size size)
		noexcept(std::is_nothrow_invocable_v<
			decltype(&t_This::pop_back),
			t_This>)
	{
		runtimeAssert(size >= 0 && size <= Capacity, "Bad Resize");
